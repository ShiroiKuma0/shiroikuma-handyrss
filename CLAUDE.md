...
- **`$HOME/.handyrss_build_no`** — the build counter. **Currently `45`** (last built version `1.1.4+45`; `+44` was burnt by a failed build — missing `local.properties`). Never commit.
...
- HEAD = `541dfb02` (`Rebase onto upstream master @ 726fefe3; build 1.1.4+45`). Last built + on-device-confirmed `1.1.4+45`, counter at `45`. Latest published GitHub release is still `1.1.4+41` (APK attached, default branch `custom`).
- All commits pushed to `origin/custom` (force-pushed after the rebase). Tree clean.
...
- **Recent feature commits (34–36, `1.1.4+42..+45`; full detail in the skill's feature-commits list):** uniform grid-card height (fixed title line count, reserved day-header line, image size folded into the grid date line); mark entries read on leaving the reading view even when starred (dropped the `!mFavorite` guard + `WHERE_NOT_FAVORITE` clause); the 2026-07-24 upstream master-tip rebase itself.
