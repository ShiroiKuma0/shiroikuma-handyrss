...
- **`$HOME/.handyrss_build_no`** — the build counter. **Currently `9`** (last built version `1.1.5+9`; reset to 0 when the base moved `1.1.4` → `1.1.5`). Never commit.
...
## State checkpoint (2026-07-28)

- Last built + pushed `1.1.5+9`, counter at `9` (`+6` export completeness, `+7` `CANCEL_EXPORT`, `+8` the ghost sweep, `+9` progress `item`/heartbeat/bytes + the export moved into a foreground service).
