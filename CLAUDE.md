...
- **`$HOME/.handyrss_build_no`** — the build counter. **Currently `8`** (last built version `1.1.5+8`; reset to 0 when the base moved `1.1.4` → `1.1.5`). Never commit.
...
- Last built + pushed `1.1.5+8`, counter at `8` (`+6` export completeness, `+7` `CANCEL_EXPORT`, `+8` the ghost sweep).
...
- **Recent feature commits (34–42; full detail in the skill's feature-commits list):** the state ZIP now exports articles, downloaded text/images and the font files, and the export is cancellable (`CANCEL_EXPORT`, `.part` + rename); orphaned images/article files are reaped on feed and group deletion, and the delete-old job no longer requires the charger. Earlier: uniform grid-card height; mark entries read on leaving the reading view even when starred; the UI-screen kxkb restyle + category Export/Import panel (`1.1.4+46`); the 2026-07-25 rebase + `1.1.5` base adoption; **long-press the top-right ⋮ in the entry list → 白い熊 Handy RSS UI screen** (`EntriesListFragment.SetupMoreOptionsLongPress`, third entry point beside the drawer settings icon and the top-left actionbar icon).
